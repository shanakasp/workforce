...
test('employee can create a workflow item while admin and reviewer participate in the flow', () => {
  ...
  const employee = service.createUser({ name: 'Employee', email: 'employee@example.com', roles: ['employee'] });
  ...
  const transitionedItem = service.transitionItem(item.id, {
    userId: reviewer.id,
    toStageId: 'review'
  });
